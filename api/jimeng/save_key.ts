...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const key = resolveJimengApiKey(req.headers['x-jimeng-api-key'] || req.body?.api_key)
  ...
  try {
    ...
    if (response.ok) return res.status(200).json({ ok: true, message: 'API Key 已在当前浏览器保存并验证' })
