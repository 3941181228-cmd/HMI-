...
/**
 * 缩放弹簧
 * 专为中央卡片的"点击/轻触"视觉反馈设计的弹性更强的弹簧
 */
const TAP_SPRING = {
  type: "spring",
  stiffness: 450,
  damping: 18, // 阻尼越低 = 微妙的过冲/摆动"轻触"效果
  mass: 1,
} as const;
...
export function FocusRail({
  ...
  const onWheel = React.useCallback(
    (e: React.WheelEvent) => {
      const now = Date.now();
      // 防抖：防止惯性滚动导致的快速触发（400ms 锁定）
      if (now - lastWheelTime.current < 400) return;

      // 优先检测水平滚动，按住 Shift 时回退到垂直滚动
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                ...
                // 使用按属性区分的弹簧：default 覆盖所有属性，scale 单独使用更具弹性的 TAP_SPRING
                transition={{
                  default: BASE_SPRING,
                  scale: TAP_SPRING,
                }}
