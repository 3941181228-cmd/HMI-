...
export default async function handler(event: any) {
  ...
  const candidate = event.headers?.['x-jimeng-api-key'] || body.api_key
  ...
  try {
    ...
    return { statusCode: 200, body: JSON.stringify({ ok, message: ok ? (candidate ? 'API Key 已在当前浏览器保存并验证' : '默认 API 已验证') : response.status === 401 || response.status === 403 ? 'API Key 无效或已过期' : 'API 暂时无法验证，请稍后重试' }) }
  } catch {
